...
return new class extends Migration
{
    public function up(): void
    {
        Schema::table('company_credit_note_items', function (Blueprint $table) {
            if (! Schema::hasColumn('company_credit_note_items', 'is_taxable')) {
                $table->boolean('is_taxable')->default(false)->after('item_amount');
            }

            if (! Schema::hasColumn('company_credit_note_items', 'tax_id')) {
                $table->unsignedBigInteger('tax_id')->nullable()->after('is_taxable');
            }

            if (! Schema::hasColumn('company_credit_note_items', 'tax_item_name')) {
                $table->string('tax_item_name')->nullable()->after('tax_id');
            }

            if (! Schema::hasColumn('company_credit_note_items', 'item_type')) {
                $table->enum('item_type', ['fixed', 'percent'])->nullable()->after('tax_item_name');
            }

            if (! Schema::hasColumn('company_credit_note_items', 'item_value')) {
                $table->decimal('item_value', 15, 2)->nullable()->after('item_type');
            }

            if (! Schema::hasColumn('company_credit_note_items', 'tax_amount')) {
