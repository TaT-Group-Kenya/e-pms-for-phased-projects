...
<head>
    ...
    <meta charset="UTF-8">
    <title>Statement of account - {{ $meta['customer_name'] ?? '' }}</title>
    <style>
                    ...
                    <div style="margin-top: 16px;">
                        ...
                        <div class="text-sm">
                            @if($meta['customer_name'])
                                <span class="font-semibold">{{ $meta['customer_name'] }}</span>
                            @else
                                -
                            @endif
                        </div>
                        @if($meta['customer'])
                            @if(!empty($meta['customer']['address']))
                                <div class="text-sm mt-1">{{ $meta['customer']['address'] }}</div>
                            @endif
                            @if(!empty($meta['customer']['city']) || !empty($meta['customer']['state']) || !empty($meta['customer']['country']))
                                <div class="text-sm mt-1">
                                    {{ trim(($meta['customer']['city'] ?? '') . (isset($meta['customer']['city'], $meta['customer']['state']) ? ', ' : '') . ($meta['customer']['state'] ?? '')) }}
                                    @if(!empty($meta['customer']['country']))
                                        {{ isset($meta['customer']['city']) || isset($meta['customer']['state']) ? ', ' : '' }}{{ $meta['customer']['country'] }}
                                    @endif
                                </div>
                            @endif
                            @if(!empty($meta['customer']['phone']))
                                <div class="text-sm mt-1">Phone: {{ $meta['customer']['phone'] }}</div>
                            @endif
                            @if(!empty($meta['customer']['email']))
                                <div class="text-sm mt-1">Email: {{ $meta['customer']['email'] }}</div>
                            @endif
                        @endif
                    </div>
                </td>
