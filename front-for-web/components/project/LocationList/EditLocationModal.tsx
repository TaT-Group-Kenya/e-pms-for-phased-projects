...
interface EditLocationModalProps {
  locationId: number;
  location: { id: number; code?: string | null; name: string; description?: string | null };
  onClose: () => void;
  onSuccess: () => void;
}

const EditLocationModal: React.FC<EditLocationModalProps> = ({ locationId, location, onClose, onSuccess }) => {
  ...
  const onSubmit = async (data: LocationFormData) => {
    ...
    try {
      const response = await fetch(`/api/projects/locations/${locationId}`, {
        method: "PUT",
        headers: {
      ...
      if (!response.ok) {
        ...
        setError(`Failed to update project location: ${formattedError}`);
        return;
      }

      addToast("Project location updated successfully", "success");
      onSuccess();
    } catch (err) {
      console.error("Error updating project location:", err);
      setError("Failed to update the project location. Please check your connection and try again.");
    } finally {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-[20px]">
      <div className="bg-white dark:bg-[#0c1427] rounded-md p-[25px] w-full max-w-[500px]">
        <div className="mb-[20px]">
          <h6 className="!mb-0">Edit Project Location</h6>
        </div>
