...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const url = `${base}/quotations/${id}/send-email`;
    ...
    if (!resp.ok) {
      const message = data?.message || `Failed to send quotation email (status ${resp.status})`;
      return res.status(resp.status).json({ message, details: data });
    }

    return res.status(200).json(data || { message: "Quotation emailed to customer successfully." });
  } catch (err) {
