  ...
  try {
    ...
    if (!resp.ok) {
      try {
        ...
        const message =
          errorBody?.message || `Failed to download company invoice PDF (status ${resp.status})`
        ...
        const message =
          text || `Failed to download company invoice PDF (status ${resp.status})`
    ...
    const contentDisposition =
      ...
      `attachment; filename="company-invoice-${id}.pdf"`
