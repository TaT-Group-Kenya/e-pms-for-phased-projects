...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (req.method === 'DELETE') {
      ...
      if (!resp.ok) {
        return res.status(resp.status).json(data || { message: 'Failed to delete company invoice' })
      }

      return res.status(resp.status).json(data || { message: 'Company invoice deleted successfully' })
    }
  } catch (err) {
