...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (!resp.ok) {
      const message = data?.message || `Failed to send company invoice email (status ${resp.status})`
      return res.status(resp.status).json({ message, details: data })
    }

    return res.status(200).json(data || { message: 'Company invoice emailed successfully.' })
  } catch (err) {
    // eslint-disable-next-line no-console
    console.error('send company invoice email error', err)
