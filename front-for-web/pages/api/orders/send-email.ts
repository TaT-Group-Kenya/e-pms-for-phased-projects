...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const url = `${base}/orders/${id}/send-email`;
    ...
    if (!resp.ok) {
      const message = data?.message || `Failed to send order email (status ${resp.status})`;
      return res.status(resp.status).json({ message, details: data });
    }

    return res.status(200).json(data || { message: "Order emailed to customer successfully." });
  } catch (err) {
