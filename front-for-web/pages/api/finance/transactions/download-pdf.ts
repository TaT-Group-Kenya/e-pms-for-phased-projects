...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const url = `${base}/transactions/${id}/download-pdf`;
    ...
    if (!resp.ok) {
      try {
        ...
        const message =
          errorBody?.message || `Failed to download transaction PDF (status ${resp.status})`;
        ...
        const message = text || `Failed to download transaction PDF (status ${resp.status})`;
