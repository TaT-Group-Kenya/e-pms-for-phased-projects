...
export default function TaxPaymentsCustomerReportPage() {
  ...
  const fetchTransactions = async () => {
    ...
    try {
      ...
      const url = `/api/reports/tax-payments-customer${params.toString() ? `?${params.toString()}` : ""}`;
  ...
  const exportCsv = () => {
    ...
    const rows = transactions.map((t: any) => [
      ...
      t.tax_amount,
      t.exchange_rate,
      t.converted_tax_amount,
      t.created_at ? new Date(t.created_at).toLocaleDateString() : "",
    ]);
    // Totals rows
    let totalsRows: string[][] = [];
    if (totals && typeof totals.taxes !== "undefined") {
      totalsRows.push(["", "", "", "", "Total Taxes", "", "", ""]);
      totalsRows.push(["", "", "", "", totals.taxes, "", "", ""]);
    }
    if (totals && typeof totals.converted_taxes !== "undefined") {
      totalsRows.push(["", "", "", "", "Converted Total Taxes", "", "", ""]);
      totalsRows.push(["", "", "", "", totals.converted_taxes, "", "", ""]);
    }
    const csv = [header, ...rows, ...totalsRows].map((r) => r.join(",")).join("\n");
    const blob = new Blob([csv], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "tax-payments-customer.csv";
    a.click();
  ...
  const exportPdf = () => {
    ...
    params.append("reportType", "taxPaymentsCustomer");
    ...
    fetch(url, {
      ...
      .then((blob) => {
        ...
        const a = document.createElement("a");
        a.href = url;
        a.download = "tax-payments-customer.pdf";
        a.click();
        URL.revokeObjectURL(url);
      })
  ...
  return (
    <>
      ...
      <AuthenticatedLayout>
        <Can any={["ROLE_VIEW_CUST_PAYMENT", "ROLE_VIEW_TAX"]} fallback={<div>You do not have permission to view this report.</div>}>
          {/* Card 1: Title & Export */}
          <div className="mb-6">
            <div className="bg-white rounded-lg shadow p-5 flex items-center justify-between">
              <h5 className="!mb-0">Tax Payments (Customer) Report</h5>
          ...
          <div className="bg-white rounded-lg shadow p-5 mb-[5px]">
            {!hasRun ? (
              <div className="flex flex-col items-center justify-center py-12">
                ...
                <p className="text-gray-500 mb-4 text-center">No data has been requested yet.<br />Set your filters and click <span className='font-semibold text-primary-500'>Run Report</span> to generate your tax payments (customer) report.</p>
                  ...
                  <table className="table table-bordered table-sm min-w-max w-full">
                    <thead>
                      <tr>
                        ...
                        <th className="whitespace-nowrap min-w-[120px] text-left px-4 py-2">Exchange Rate</th>
                        <th className="whitespace-nowrap min-w-[120px] text-left px-4 py-2">Converted Tax</th>
                      ...
                      {loading ? (
                        ...
                        transactions.map((t: any, idx: number) => (
                          <tr key={t.id} className={idx % 2 === 0 ? "bg-gray-50 border-b border-gray-200" : "bg-white border-b border-gray-200"}>
                            ...
                            <td className="whitespace-nowrap min-w-[120px] text-left px-4 py-2">{t.customer_name}</td>
                            ...
                            <td className="whitespace-nowrap min-w-[120px] text-left px-4 py-2">{t.exchange_rate}</td>
                            <td className="whitespace-nowrap min-w-[120px] text-left px-4 py-2">{t.converted_tax_amount}</td>
          ...
          {totals && typeof totals.taxes !== "undefined" && (
            <div className="bg-white rounded-lg shadow p-5 mt-6 max-w-full mb-[25px]">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-gray-600">Converted Total Taxes:</span>
                <span className="font-bold">{formatCurrency(totals.converted_taxes, 'KES')}</span>
              </div>
            </div>
          )}
