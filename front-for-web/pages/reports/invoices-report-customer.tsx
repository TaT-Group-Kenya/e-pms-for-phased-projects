...
export default function InvoicesReportCustomerPage() {
    ...
    fetch('/api/currencies/list', { headers: { Authorization: `Bearer ${accessToken}` } })
      ...
      .then(result => {
        ...
        if (result.ok && Array.isArray(result.data)) {
          ...
          if (!filters.currency_code && result.data.length > 0) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const url = `/api/reports/invoices-customer${params.toString() ? `?${params.toString()}` : ''}`;
  ...
  const exportPdf = async () => {
    try {
      ...
      params.append('reportType', 'invoicesReportCustomer');
      ...
      a.href = urlObj;
      a.download = 'invoices-report-customer.pdf';
  ...
  const exportCsv = () => {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = 'invoices-report-customer.csv';
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
        ...
        <Can any={["ROLE_VIEW_COMPANY_INVOICE", "ROLE_VIEW_CUST_INVOICE"]} fallback={<div>You do not have permission to view this report.</div>}>
          ...
          <div className="mb-6">
            <div className="bg-white rounded-lg shadow p-5 flex items-center justify-between">
              <h5 className="!mb-0">Invoices Report Customer</h5>
          ...
          <div className="bg-white rounded-lg shadow p-5 mb-[25px]">
            {!hasRun ? (
              <div className="flex flex-col items-center justify-center py-12">
                ...
                <p className="text-gray-500 mb-4 text-center">No data has been requested yet.<br />Set your filters and click <span className='font-semibold text-primary-500'>Run Report</span> to generate your invoices customer report.</p>
