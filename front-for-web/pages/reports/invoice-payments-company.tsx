...
export default function InvoicePaymentsCompanyReportPage() {
  ...
  useEffect(() => {
    ...
    ]).then(([currencies, companies]) => {
      if (!isMounted) return;
      if (currencies.ok && Array.isArray(currencies.data)) {
        const kesOnly = currencies.data.filter((c: any) => (c.code || c.id || c.value) === 'KES');
        setCurrencyOptions(kesOnly);
        if (!filters.currency_code && kesOnly.length > 0) {
          const first = kesOnly[0];
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const url = `/api/reports/invoice-payments-company${params.toString() ? `?${params.toString()}` : ''}`;
  ...
  async function exportPdf() {
    try {
      ...
      params.append('reportType', 'invoicePaymentsCompany');
      ...
      a.href = urlObj;
      a.download = 'invoice-payments-company.pdf';
  ...
  function exportCsv() {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = 'invoice-payments-company.csv';
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
        ...
        <Can any={["ROLE_VIEW_COMPANY_INVOICE", "ROLE_VIEW_COMPANY_PAYMENT", "ROLE_VIEW_CUST_INVOICE", "ROLE_VIEW_CUST_PAYMENT"]} fallback={<div>You do not have permission to view this report.</div>}>
          ...
          <div className="mb-6">
            <div className="bg-white rounded-lg shadow p-5 flex items-center justify-between">
              <h5 className="!mb-0">Invoice Payments Company Report</h5>
          ...
          <div className="bg-white rounded-lg shadow p-5 mb-[25px]">
            {!hasRun ? (
              <div className="flex flex-col items-center justify-center py-12">
                ...
                <p className="text-gray-500 mb-4 text-center">No data has been requested yet.<br />Set your filters and click <span className='font-semibold text-primary-500'>Run Report</span> to generate your invoice payments company report.</p>
                ...
                <table className="table table-bordered table-sm min-w-max w-full">
                  <thead>
                    <tr>
                      ...
                      <th className="whitespace-nowrap min-w-[160px] text-left px-4 py-2">Company</th>
                    ...
                    {loading ? (
                      ...
                      data.map((row, idx) => (
                        <tr key={idx} className={idx % 2 === 0 ? 'bg-gray-50 border-b border-gray-200' : 'bg-white border-b border-gray-200'}>
                          ...
                          <td className="whitespace-nowrap text-left px-4 py-2">{row.company_name ?? ''}</td>
