...
export default function CustomerStatementReportPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([customersRes, currenciesRes]) => {
        if (!isMounted) return;
        const customerList = Array.isArray(customersRes.data) ? customersRes.data : Array.isArray(customersRes) ? customersRes : [];
  ...
  const validateFilters = (): boolean => {
    if (!filters.customer_id || !filters.currency_code || !filters.from || !filters.to) {
      const msg = 'Customer, currency, from and to date filters are required.';
      setError(msg);
      addToast(msg, 'error');
      return false;
    }
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const url = `/api/reports/customer-statement${params.toString() ? `?${params.toString()}` : ''}`;
      ...
      if (resp.ok) {
        ...
        const msg = result.error || 'Failed to fetch customer statement';
        setError(msg);
        addToast(msg, 'error');
      }
    } catch (err) {
  ...
  const exportPdf = async () => {
    ...
    try {
      ...
      params.append('reportType', 'customerStatement');
  ...
  const exportCsv = () => {
    ...
    const columns = ['Date', 'Job Ref', 'Document #', 'Customer', 'Description', 'Debit', 'Credit', 'Balance'];
  ...
  return (
    <>
      ...
      <AuthenticatedLayout>
        <Can any={["ROLE_VIEW_CUSTOMER_TRANSACTIONS_LEDGER"]} fallback={<div>You do not have permission to view this report.</div>}>
          <div className="mb-6">
            <div className="bg-white rounded-lg shadow p-5 flex items-center justify-between">
              <div>
                <h5 className="!mb-0">Customer Statement</h5>
                <p className="text-sm text-gray-500">Generate account statements for customers with CSV and PDF export.</p>
              </div>
