...
const CustomerPaymentDetailPageInner: React.FC = () => {
  ...
  const [entry, setEntry] = useState<CustomerPaymentSummary | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchDetail = async () => {
      setLoading(true);
      try {
        const resp = await fetch(`/api/finance/customer-payments/${idParam}`, {
          method: "GET",
          headers: {
        ...
        if (!resp.ok) {
          addToast(data?.message || "Failed to load customer payment", "error");
          setEntry(null);
          return;
        }
        ...
        console.error("customer payment detail error", err);
        addToast("Error loading customer payment.", "error");
        setEntry(null);
      } finally {
        setLoading(false);
      }
  ...
  const handleExportPdf = async () => {
    ...
    try {
      const resp = await fetch(
        `/api/finance/customer-payments/download-pdf?id=${entry.id}`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${accessToken}`,
      ...
      link.href = url;
      link.download = `customer-payment-${
      ...
      // eslint-disable-next-line no-console
      console.error("customer payment pdf error", err);
...
export default function CustomerPaymentDetailPage() {
  return (
    <AuthenticatedLayout>
      <CustomerPaymentDetailPageInner />
    </AuthenticatedLayout>
  );
}
