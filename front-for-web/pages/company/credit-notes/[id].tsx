...
interface CompanyCreditNoteTaxItem {
  id: number
...
const CompanyCreditNoteDetailPage: React.FC = () => {
  ...
  const [taxItems, setTaxItems] = useState<CompanyCreditNoteTaxItem[]>([])
  ...
  const fetchCreditNote = async () => {
    ...
    try {
      ...
      const mapped: CompanyCreditNote = {
        ...
        exchange_rate: Number(cn.exchange_rate ?? 1),
  ...
  const fetchTaxItems = async () => {
    ...
    try {
      const resp = await fetch(`/api/company-credit-note-tax-items?credit_note_id=${id}` as string, {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
      ...
      const mapped: CompanyCreditNoteTaxItem[] = (items || []).map((t: any) => ({
        id: t.id,
    ...
    <AuthenticatedLayout>
      ...
      <div className="mb-[25px] md:flex items-center justify-between">
        <div className="mb-[15px] md:mb-0">
          <h5 className="!mb-1">
            Company Credit Note {creditNote?.credit_note_number || `#${creditNote?.id}`}
          </h5>
          <p className="text-gray-500 dark:text-gray-400 text-sm">
            Overview and tax breakdown for this company credit note.
          </p>
        </div>
        ...
        <div className="space-y-[8px] text-right">
          <ul className="flex flex-wrap items-center gap-[10px] text-sm justify-end">
            ...
            <li>
              <Link href="/company/credit-notes" className="text-gray-500 dark:text-gray-400">
          ...
          {creditNote?.invoice_id && (
            <div>
              <Link
                href={`/company/invoices/${creditNote.invoice_id}`}
          ...
          {activeTab === 'overview' && creditNote && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-[20px]">
              <div className="space-y-[10px]">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-gray-500 dark:text-gray-400 text-sm">Credit Note Date</span>
                  <span className="font-medium text-sm">
