                ...
                <table className="w-full">
                  ...
                  <tbody className="text-black dark:text-white">
                    {(items || []).filter((it) => {
                      ...
                      return (`${it.invoice_number || ''}`.toLowerCase().includes(st) || `${it.company_name || it.company?.name || ''}`.toLowerCase().includes(st) || `${it.cheque_number || ''}`.toLowerCase().includes(st))
                    }).map((it: any) => (
                      <tr key={it.id} className="border-b border-gray-100 dark:border-[#172036] hover:bg-gray-50 dark:hover:bg-[#15203c] transition-colors">
                        <td className="ltr:text-left rtl:text-right px-[20px] py-[15px] whitespace-nowrap">
                          {it.invoice ? (
                            <Link href={`/company/invoices/${it.invoice.id}`} className="text-primary-500 hover:text-primary-600 hover:underline font-medium text-sm">{it.invoice.invoice_number}</Link>
                          ) : (
